            ...
            {parsed.map((t, i) => (
              <div key={i} style={{ background: 'rgba(255,255,255,0.025)', border: `1px solid rgba(255,255,255,0.06)`, borderRadius: 9, padding: '12px 14px' }}>
                <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                  <input style={{ ...inputStyle, flex: 2, fontSize: 12.5 }} value={t.title} onChange={e => updateParsed(i, 'title', e.target.value)} placeholder="Nome do assunto" />
                  <input style={{ ...inputStyle, flex: 1, fontSize: 12.5 }} value={t.subtitle} onChange={e => updateParsed(i, 'subtitle', e.target.value)} placeholder="Detalhe (opcional)" />
...
function parseTopics(text) {
  ...
  // Tenta separar por ponto-e-vírgula, bullet, numeração, ou quebra de linha
  if (text.includes(';')) {
    items = text.split(';').map(s => s.trim()).filter(s => s.length > 2)
  } else if (/\n/.test(text) && text.split('\n').filter(l => l.trim().length > 2).length > 1) {
    items = text.split('\n').map(l => l.replace(/^[\d\.\-\•\*\)\s]+/, '').trim()).filter(s => s.length > 2)
  } else if (/,/.test(text) && text.split(',').length > 2) {
    items = text.split(',').map(s => s.trim()).filter(s => s.length > 2)
  } else {
    // Texto corrido: divide por conectores comuns
    items = text.split(/;\s*|\n+|(?<=\w),(?=\s+[A-ZÁÉÍ])/).map(s => s.replace(/^[\d\.\-\•\*\)\s]+/, '').trim()).filter(s => s.length > 2)
    if (items.length < 2) items = [text.trim()] // mantém como 1 tópico
  }

  return items.slice(0, 30).map((raw, i) => {
    const title = raw.slice(0, 100).trim()
    const diff = inferDiff(title)
    const weight = inferWeight(title)
    const hours = diff === 'hard' ? 4 : diff === 'medium' ? 2 : 1
    return { title, subtitle: '', weight, difficulty: diff, estimated_hours: hours }
  })
}
