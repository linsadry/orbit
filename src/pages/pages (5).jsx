                    ...
                    {topic.subtopics?.length > 0 && (
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 6 }}>
                        {(typeof topic.subtopics[0] === 'string' ? topic.subtopics : []).slice(0, 5).map((s, j) => (
                          <span key={j} style={{ fontSize: 10.5, padding: '2px 7px', borderRadius: 99, border: '1px solid rgba(255,255,255,0.07)', color: C.text3 }}>{s}</span>
                        ))}
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
          <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
            ...
            {['easy', 'medium', 'hard'].map(d => (
              <button key={d} onClick={() => setManDiff(d)} style={{ padding: '4px 10px', borderRadius: 6, border: `1px solid ${manDiff === d ? dc[d] + '88' : 'rgba(255,255,255,0.07)'}`, background: manDiff === d ? dc[d] + '18' : 'none', color: manDiff === d ? dc[d] : C.text3, fontSize: 11.5, cursor: 'pointer', fontFamily: "'Geist', system-ui, sans-serif" }}>{dl[d]}</button>
            ))}
          </div>
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <span style={{ fontSize: 11, color: C.text3 }}>Peso:</span>
            {[5, 10, 15, 20, 25].map(w => (
              <button key={w} onClick={() => setManWeight(w)} style={{ padding: '4px 8px', borderRadius: 6, border: `1px solid ${manWeight === w ? orbit.color + '88' : 'rgba(255,255,255,0.07)'}`, background: manWeight === w ? orbit.color + '18' : 'none', color: manWeight === w ? orbit.color : C.text3, fontSize: 11.5, cursor: 'pointer', fontFamily: "'Geist', system-ui, sans-serif" }}>{w}</button>
            ))}
          </div>
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <span style={{ fontSize: 11, color: C.text3 }}>Horas:</span>
            {[1, 2, 4, 6, 8].map(h => (
              <button key={h} onClick={() => setManHours(h)} style={{ padding: '4px 8px', borderRadius: 6, border: `1px solid ${manHours === h ? orbit.color + '88' : 'rgba(255,255,255,0.07)'}`, background: manHours === h ? orbit.color + '18' : 'none', color: manHours === h ? orbit.color : C.text3, fontSize: 11.5, cursor: 'pointer', fontFamily: "'Geist', system-ui, sans-serif" }}>{h}h</button>
            ))}
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button style={{ padding: '8px 14px', background: 'none', border: `1px solid rgba(255,255,255,0.07)`, borderRadius: 8, color: C.text2, fontSize: 13, cursor: 'pointer', fontFamily: "'Geist', system-ui, sans-serif" }} onClick={onCancel}>Cancelar</button>
        </div>
      </div>
    )
  }
      ...
      {step === 'review' && (
        <>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16, maxHeight: 400, overflowY: 'auto' }}>
            {parsed.map((t, i) => (
              <div key={i} style={{ background: 'rgba(255,255,255,0.025)', border: `1px solid rgba(255,255,255,0.06)`, borderRadius: 9, padding: '12px 14px' }}>
                ...
                {t.subtopics?.length > 0 && (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 8 }}>
                    {t.subtopics.slice(0, 6).map((s, j) => (
                      <span key={j} style={{ fontSize: 10.5, padding: '2px 8px', borderRadius: 99, border: '1px solid rgba(255,255,255,0.07)', color: C.text3 }}>{s}</span>
                    ))}
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
                  ...
                  {['easy', 'medium', 'hard'].map(d => (
                    <button key={d} onClick={() => updateParsed(i, 'difficulty', d)} style={{ padding: '3px 8px', borderRadius: 99, border: `1px solid ${t.difficulty === d ? dc[d] + '88' : 'rgba(255,255,255,0.07)'}`, background: t.difficulty === d ? dc[d] + '18' : 'none', color: t.difficulty === d ? dc[d] : C.text3, fontSize: 10.5, cursor: 'pointer', fontFamily: "'Geist', system-ui, sans-serif" }}>{dl[d]}</button>
                  ))}
                  <span style={{ fontSize: 10.5, color: C.text3, marginLeft: 8, marginRight: 2 }}>Peso:</span>
                  {[5, 10, 15, 20].map(w => (
                    <button key={w} onClick={() => updateParsed(i, 'weight', w)} style={{ padding: '3px 7px', borderRadius: 99, border: `1px solid ${t.weight === w ? orbit.color + '88' : 'rgba(255,255,255,0.07)'}`, background: t.weight === w ? orbit.color + '18' : 'none', color: t.weight === w ? orbit.color : C.text3, fontSize: 10.5, cursor: 'pointer', fontFamily: "'Geist', system-ui, sans-serif" }}>{w}</button>
                  ))}
                  <span style={{ fontSize: 10.5, color: C.text3, marginLeft: 8, marginRight: 2 }}>Horas:</span>
                  {[1, 2, 4, 6].map(h => (
                    <button key={h} onClick={() => updateParsed(i, 'estimated_hours', h)} style={{ padding: '3px 7px', borderRadius: 99, border: `1px solid ${t.estimated_hours === h ? orbit.color + '88' : 'rgba(255,255,255,0.07)'}`, background: t.estimated_hours === h ? orbit.color + '18' : 'none', color: t.estimated_hours === h ? orbit.color : C.text3, fontSize: 10.5, cursor: 'pointer', fontFamily: "'Geist', system-ui, sans-serif" }}>{h}h</button>
                  ))}
                </div>
              </div>
            ))}
...
function parseTopics(text) {
  // Tenta detectar o formato "Matéria: conteúdo..." (formato edital RFB)
  ...
  if (asMaterias.length >= 1) {
    // Formato edital: "Matéria: 1. item 2. item..."
  ...
  // Formato lista simples: separado por ;  ou  linhas  ou  vírgula
  let items = []
  if (text.includes(';')) {
...
function extractSubs(body) {
  const subs = []
  // Extrai itens numerados: "1. Conceitos 2. Demanda..."
  ...
  // Fallback: separa por ponto-e-vírgula ou ponto se sem numeração
  if (subs.length === 0) {
